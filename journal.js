...
function populateJournalEntries() {
  ...
  Object.keys(journalEntries).forEach(entryId => {
    ...
    entryElement.className = 'relative bg-bg-primary p-1.5 rounded border border-text-heading hover:border-text-primary cursor-pointer journal-entry';
...
function openEntryModal(entryId) {
  ...
  if (modalTitle && modalContent && entryModal) {
    // Get the entry data from our journalEntries object
    const entry = journalEntries[entryId];
    ...
    if (entry) {
      ...
      modalContent.innerHTML = `
        <div class="text-text-primary">
          <div class="text-sm text-text-heading mb-2">${entry.date}</div>
          ${entry.content}
        </div>
      `;
    } else {
